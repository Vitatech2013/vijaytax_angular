...
export class TableReportsComponent implements OnInit {
  ...
  constructor(
    ...
    private tableService: TablesService, public datepipe: DatePipe
  ) { }
  ...
  async loadData() {
    ...
    this.orderService.tableOrderReports(this.rangeForm.value.start, this.rangeForm.value.end, this.rangeForm.value.tableId).subscribe(data => {
  ...
  getFileName = (name: string) => {
    ...
    let sheetName = name || "TableReports";
  ...
  exportTable() {
    if (this.dataSource.filteredData.length === 0) {
      ...
      XLSX.writeFile(workBook, `TableReports.xlsx`);
    }
